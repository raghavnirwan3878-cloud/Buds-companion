...
public class BudsConnectionService extends Service {
    ...
    public void onDestroy() {
        ...
        getSharedPreferences(PREFS, MODE_PRIVATE).edit().putBoolean(PREF_CONNECTED, false).apply();
        closeSocketQuietly();
        super.onDestroy();
    }
    ...
    private void applyBatteryUpdate(List<OppoProtocol.BatteryInfo> batteries) {
        ...
        for (OppoProtocol.BatteryInfo b : batteries) {
            if (b.index == 2) {
                // Case: only meaningful while it's actually reporting (open).
                // A 0% case reading is already filtered out upstream in
                // OppoProtocol, so anything that reaches here is a real,
                // fresh value - stamp it so staleness can hide it later.
                editor.putInt(PREF_CASE, b.level);
