 ...
 *
 * This intentionally bypasses Gadgetbridge/realme Link entirely - it talks
 * the vendor protocol directly, based on Gadgetbridge's (AGPLv3) open-source
...
public class BudsConnectionService extends Service {
    ...
    // honor our push-subscription request; kept short so battery still shows
    // up quickly and stays fresh.
        ...
        new Thread(() -> {
            while (running) {
                try {
                    ...
                    stopSelf(); // remove the notification - nothing more we can do
                    return;
                } catch (Exception e) {
                ...
                if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
                    // The earbuds are supposed to be Bluetooth-connected right
                    // now (that's what started this service), but we still
                    // can't open the RFCOMM channel after several tries -
                    // give up rather than sit forever with a live notification.
                    Log.w(TAG, "Giving up after " + consecutiveFailures + " failed attempts");
                    stopSelf();
                    return;
                }
